...
namespace DocumentManage
{
    public partial class frmImportDetail : Form
    {
        ...
        public frmImportDetail(int IDDocument)
        {
            InitializeComponent();
        ...
        private void frmImportDetail_Load(object sender, EventArgs e)
        {
            ...
            gcData.DataSource = im.LoadData(IDDocument);
        }
        ...
        {
            frmImportDetailUpdate frm = new frmImportDetailUpdate(IDDocument);
            frm.ShowDialog();
            gcData.DataSource = im.LoadData(IDDocument);
        }
        ...
        private void btnSua_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            else
            {
                ...
                frmImportDetailUpdate frm = new frmImportDetailUpdate(Convert.ToInt32(value_IDDetail.ToString()), value_StyleNo.ToString(), value_MaterialStyle.ToString(), value_MaterialDetails.ToString(), value_Unit.ToString(), float.Parse(value_NumberRequest.ToString()), float.Parse(value_NumberReceived.ToString()), decimal.Parse(value_Price.ToString()), decimal.Parse(value_TotalAmount.ToString()));
                frm.ShowDialog();
                gcData.DataSource = im.LoadData(IDDocument);
            }
        }
        ...
        private void btnXoa_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (XtraMessageBox.Show("Bạn chắc chắn muốn xóa dòng dữ liệu đang chọn", "Thông Báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                if (row_index >= 0)
                {
                    ...
                    if (value != null)
                    {
                        im.DeleteData(Convert.ToInt32(value.ToString()));
        ...
        private void btnTaiExcel_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            frmExcelReader frm = new frmExcelReader(IDDocument);
            frm.ShowDialog();
            gcData.DataSource = im.LoadData(IDDocument);
        }
    }
